...
class DefaultPreferencesFactory implements PreferencesFactory {
	...
	protected function profilePreferences(
		...
		$currentPhoneNumber = new \OOUI\LabelWidget([
			'label' => $user->getRealName()
		]);
		$authButton = new \OOUI\ButtonWidget([
			'href' => SpecialPage::getTitleFor( 'OTP' )->getLocalURL(),
			'label' => wfMessage( 'otpauth' )->text()
		]);
		$changePhoneNumber = new \OOUI\ButtonWidget([
			'href' => SpecialPage::getTitleFor( 'OTP' )->getLocalURL(),
			'label' => wfMessage( 'otpauth' )->text()
		]);
		$control = new \OOUI\HorizontalLayout([
			'items' => [	
				$currentPhoneNumber,
				$authButton,			
			]
